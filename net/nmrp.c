...
/*
Enter nmrp mode：
Power LED solid Red，WPS LED solid Green
Other off
 
Reworking：
Power LED solid Red, WPS LED blinking Green（0.25 sec on and 0.75 sec off）
Other off
 
Upgrade success：
...
{
    led_set_state(WPS_GREEN_SELECTOR, STATUS_LED_ON, 0, 0);
...
{
    led_set_state(WPS_GREEN_SELECTOR, STATUS_LED_BLINKING, 250, 750);
    led_set_state(POWER_RED_SELECTOR, STATUS_LED_ON, 0, 0);
    led_set_state(POWER_GREEN_SELECTOR, STATUS_LED_OFF, 0, 0);
}
...
{
    led_set_state(WPS_GREEN_SELECTOR, STATUS_LED_OFF, 0, 0);
    if (result == 0)
    {
...
void nmrp_set_tftp_upload_string_table_status(int nStatus, int nIndex)
{
    if (nStatus == 0)
    {
