			   ...
			   void *data)
{
	...
	ret = mtk_board_flash_adjust_offset(part->flash, part->addr, offset,
					    part->addr + part->size, &offset);
	if (ret)
		return false;

	ret = mtk_board_flash_read(part->flash, part->addr + offset, size,
				   part->addr + part->size, data);
 ...
 * @return true if integrity verification passed
 */
static bool verify_image_fit(struct di_image_info *ii, bool ram, size_t size)
{
	...
	if (!fit_all_image_verify(ii->data)) {
 ...
 * @return true if integrity verification passed
 */
static bool verify_image(struct di_image_info *ii)
{
	bool ret;
	...
	case IMAGE_FORMAT_FIT:
		return verify_image_fit(ii, false, 0);
#endif
	default:
		       ...
		       const struct di_image_info *iif)
{
	 ...
	 */
	iret = mtk_board_flash_verify_cust(iit->part.flash, iit->part.addr,
					   image_size,
					   iit->part.addr + iit->part.size,
					   iif->data, iit->data,
					   iit->part.blocksize);
	if (iret)
		goto verify_fail;
...
int dual_image_check(void)
{
	...
	bool image1_ok, image2_ok, restore_result;
	uintptr_t la, bs;
	void *flash;
	int ret;
...
#if defined(CONFIG_FIT)
	case IMAGE_FORMAT_FIT:
		bret = verify_image_fit(&iif, true, size);
		break;
#endif
	default:
		printf("Invalid image format\n");
